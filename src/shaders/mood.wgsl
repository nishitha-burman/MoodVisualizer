...
fn fs_main(in: VsOut) -> @location(0) vec4f {
  ...
  if (hit) {
    ...
    let bg_noise = fbm(vec3f(uv * 2.0, u.time * u.speed * 0.1), 3) * 0.05;
    let bg_base = oklch_to_rgb(u.brightness * 0.15, u.saturation * 0.03, u.hue + 180.0);
    let vignette = 1.0 - length(uv) * 0.4;
    color = bg_base * vignette + bg_noise;
  }
